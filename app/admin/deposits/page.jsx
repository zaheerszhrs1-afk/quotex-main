...
export default function AdminDeposits() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    fetch(`/api/admin/deposits?status=${filter}&page=${page}&limit=20`)
      .then((r) => r.json())
      .then((d) => {
        setRows(d.deposits || [])
  ...
  return (
    <div>
      <h1 className="mb-4 text-2xl font-bold">Deposit requests</h1>
      ...
      <div className="qx-card overflow-x-auto">
        <table className="w-full min-w-[880px] text-sm">
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={8} className="p-4 text-qx-textMute">Loading…</td></tr>
            )}
            {!loading && rows.length === 0 && (
              <tr><td colSpan={8} className="p-4 text-qx-textMute">No requests.</td></tr>
            )}
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-qx-border/50">
                ...
                <td className="px-4 py-3 text-qx-textDim">
                  {r.bonus ? (
                    <span className="font-semibold text-qx-green">{r.bonusCode || `${r.bonusPct}%`} +${r.bonus}</span>
                  ) : (
                    '—'
                  )}
                </td>
                <td className="px-4 py-3 text-qx-textDim">{r.senderNumber || '—'}</td>
                <td className="px-4 py-3">
                  {r.screenshotPath ? (
                    <a href={r.screenshotPath} target="_blank" rel="noreferrer" className="text-qx-green underline">
                      View
                    </a>
                  ) : (
                    '—'
